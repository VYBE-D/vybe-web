...
import { ShieldCheck, Lock, X, Mail } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

// You would pass the 'event' object as a prop to this component
export default function EventView({ event }: { event: any }) {
  ...
  const cover = event?.cover_image || "https://images.unsplash.com/photo-1534120247760-c44c3e4a62f1"; // Dark placeholder
  ...
  return (
    <>
      {/* --- EVENT CARD --- */}
      <div className="w-full max-w-md mx-auto bg-zinc-900 border border-white/10 rounded-[2rem] overflow-hidden relative group">
        
        {/* Cover Image */}
        <div className="h-64 w-full relative">
          <img src={cover} alt="Cover" className="w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity duration-500 grayscale group-hover:grayscale-0" />
        ...
        <div className="p-6 relative -mt-12 space-y-4">
          <div>
            <h2 className="text-3xl font-black italic uppercase text-white leading-none tracking-tighter mb-2">{title}</h2>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">{tagline}</p>
          </div>
